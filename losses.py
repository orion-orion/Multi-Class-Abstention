...
class HingeLoss(nn.Module):
    ...
    def forward(self, input):
        return torch.clamp(1 - self.alpha * input, 0)
...
class ExponentialLoss(nn.Module):
    ...
    def forward(self, input):
        return torch.exp(-self.alpha * input)
...
class LogisticLoss(nn.Module):
    ...
    def forward(self, input):
        return torch.log1p(self.alpha * input)
...
class PredictionMargin(nn.Module):
    ...
    def forward(self, preds, y):
        y = F.one_hot(y).bool()
        label_score = preds[y]
        max_score_excl_label = torch.max(
            preds[~y].reshape(y.shape[0], -1), dim=-1)[0]
        return self.func_phi(label_score - max_score_excl_label)
...
class PairwiseComparison(nn.Module):
    ...
    def forward(self, preds, y):
        y = F.one_hot(y).bool()
        label_score = preds[y]
        pairwise_diff_matrix = (label_score.reshape(-1, 1) - preds)[~y]
