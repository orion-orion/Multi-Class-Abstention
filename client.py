...
class Client:
    ...
    def train_epoch(self, epoch, args, global_params=None):
            ...
            for user_ids, interactions in self.train_dataloader:
                if "PPDM" in args.method:
                    batch_loss = self.trainer.train_batch(
                        user_ids, interactions, epoch, args,
        ...
        logging.info("Epoch {}/{} - client {} -  Training Loss: {:.3f}".format(
            epoch, args.num_epoch, self.c_id, loss / step))
    ...
    def cal_test_score(predictions):
        ...
        NDCG_10 = 0.0
        valid_entity = 0.0
        # `pred` indicates the rank of gepochtruth items in the recommendation
        # list
